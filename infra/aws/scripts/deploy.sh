...
configure_github_actions_aws() {
  ...
  # $18 ExistingDatabaseAppUsernamePrefix (e.g. TiDB cluster prefix; empty for RDS)
  ...
  local existing_db_app_username_prefix="${18:-}"
  ...
  if prompt_yes_no "Set environment variables for '$env_name' now (AWS_STACK_NAME, STAGE_NAME, DATABASE_SCHEMA, DB host/user vars)?" "y"; then
    ...
    if [[ "$db_mode" == "2" ]]; then
      ...
      gh variable set EXISTING_DATABASE_APP_USERNAME_PREFIX --env "$env_name" --body "$existing_db_app_username_prefix" -R "$repo"
    else
      ...
      gh variable set EXISTING_DATABASE_APP_USERNAME_PREFIX --env "$env_name" --body "" -R "$repo"
    fi
...
PREV_EXISTING_DATABASE_APP_USERNAME_PREFIX=""
...
if [[ -n "$EXISTING_STACK_STATUS" && "$EXISTING_STACK_STATUS" != "None" ]]; then
  ...
  PREV_EXISTING_DATABASE_APP_USERNAME_PREFIX="$(stack_param_value "$EXISTING_STACK_PARAMS" "ExistingDatabaseAppUsernamePrefix")"
...
ENV_EXISTING_DATABASE_APP_USERNAME_PREFIX="${EXISTING_DATABASE_APP_USERNAME_PREFIX:-}"
...
EXISTING_DATABASE_APP_USERNAME_PREFIX=""
...
if [[ "$DB_MODE" == "2" ]]; then
  ...
  EXISTING_DATABASE_APP_USERNAME_PREFIX_DEFAULT=""
  [[ -n "$PREV_EXISTING_DATABASE_APP_USERNAME_PREFIX" ]] && EXISTING_DATABASE_APP_USERNAME_PREFIX_DEFAULT="$PREV_EXISTING_DATABASE_APP_USERNAME_PREFIX"
  EXISTING_DATABASE_APP_USERNAME_PREFIX="$(resolve_with_conflict_check \
    "App username prefix (e.g. abc123. for TiDB Cloud; blank for RDS/standard)" \
    "$ENV_EXISTING_DATABASE_APP_USERNAME_PREFIX" \
    "$PREV_EXISTING_DATABASE_APP_USERNAME_PREFIX" \
    "$EXISTING_DATABASE_APP_USERNAME_PREFIX_DEFAULT")"
...
if [[ "$DB_MODE" == "2" ]]; then
  ...
  if [[ -n "$EXISTING_DATABASE_APP_USERNAME_PREFIX" ]]; then
    echo "DB app user prefix: $EXISTING_DATABASE_APP_USERNAME_PREFIX (app user: ${EXISTING_DATABASE_APP_USERNAME_PREFIX}syncbot_user_${STAGE//-/_})"
  else
    echo "DB app user prefix: (none — app user: syncbot_user_${STAGE//-/_})"
  fi
...
if [[ "$DB_MODE" == "2" ]]; then
  ...
  PARAMS+=(
    ...
    "ExistingDatabaseAppUsernamePrefix=$EXISTING_DATABASE_APP_USERNAME_PREFIX"
  )
else
  ...
  PARAMS+=(
    ...
    "ParameterKey=ExistingDatabaseAppUsernamePrefix,ParameterValue="
  )
fi
...
APP_OUTPUTS="$(app_describe_outputs "$STACK_NAME" "$REGION")"
  ...
  EXISTING_DATABASE_APP_USERNAME_PREFIX="${PREV_EXISTING_DATABASE_APP_USERNAME_PREFIX:-}"
