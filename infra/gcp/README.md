...
| `existing_db_app_username_prefix` | Optional (e.g. TiDB Cloud `abc123`). A dot separator is added automatically. When set, `DATABASE_USER` is `{prefix}.syncbot_user_{stage}` and `existing_db_user` is ignored |
